/*Méthode fetch Exercices

    1->Récupérer image dans le dossier et console.log:
...
fetch("img/image_test.jpg").then(image => {
    ...
    2->Récupèrer depuis table posts fichier 1 de l' API JSON:

fetch("https://jsonplaceholder.typicode.com/posts/1")


    3->Utiliser méthode fetch pour récupèrer un fichier text sur un serveur distant, puis afficher le dans la console
...
.catch(error => console.log(error));
    ...
    4->Ecrire une fonction qui utilise la méthode fetch pour récupèrer des données JSON à partir d' une API, puis retpourne les données sous forme d' objets JS
...
.then(function(myBlob) {
  ...
  myImage.src = objectURL;
    ...
    6->Récupèrer et afficher image du dossier dans balise image:

const myImage = document.querySelector('img');
fetch('img/image_test.jpg')
.then(function(response) {
  return response.blob();
})
.then(function(myBlob) {
  ...
  myImage.src = objectURL;
    ...
    7->Écrivez une fonction qui utilise la méthode fetch pour envoyer des données à un
serveur distant en utilisant une méthode HTTP POST:

function envoiDoJs(url, user){
...
const userTest = {
    name: "bahamut",
    mail: "bahamut-ju@gmail.com"
}
let urlTest = "https://jsonplaceholder.typicode.com/users";
envoiDoJs(urlTest, userTest);


    8->Utilisez la méthode fetch pour récupérer des données JSON à partir de l’API
themoviedb, puis utilisez ces données pour générer du contenu html en fonction
de ce que l’utilisateur aura entré comme titre dans un input que vous aurez prévu
à cet effet:
...
-await= (dans fct)attend que la fonction, requete soit exécutée pour exécuter la suite
Exemple:
Faire fonction fetchData, doit retourner nouvelle promise
function fetchData(){
    return new promise(argument => {})
}
function fetchData(){
...
fetchData();
//moins utilisée,permet chargement partiel de la page = petits paquets, que ça a fini ou non il commence a renvoyer les posts */


/* 
blob:
a mettre main js général
affichage image du dossier dans balise img HTML
...
Bien sûr, il s'agit seulement d'une réponse HTTP, pas exactement de l'image. 
Pour extraire le contenu de l'image de la réponse, nous utilisons la méthode blob() (en-US) (définie sur le mixin Body, qui est implémenté autant par les objets Request que par les objets Response).
...
/*const newUser = {
    name: "anne-so",
    mail: "anneso-ju@gmail.com"
}
// objet js a envoyer -> à tranformer en json?
const promise1 = fetch("https://jsonplaceholder.typicode.com/users",
    {
    method: "POST",
    body: JSON.stringify(newUser),
// contenu de la requête post sera dans le body, il faut tranformer l' objet JS en JSON -> foncton JSON.stringify()
// objet JSON contient des méthodes pour interpréter du JSON (javascript object notation, et convertir des valeurs en JSON.)
// JSON est une syntaxe pour sérialiser des objets, des tableaux, nombres, chaînes de caractères... basée sur syntaxe JS mais distincte.
// JSON.stringify(valeur[, remplacement[, expace]])
// ->retourne une chaîne JSON correspondante à la valeur spécifiée
// =>ici conversion objet JS en JSON
// pour envoi au serveur?
    headers: {
        "Content-Type": "application/json"
    }
});
