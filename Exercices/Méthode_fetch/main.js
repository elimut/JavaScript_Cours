/*Méthode fetch

->Exercices:

    ->Récupérer image dans le dossier et console.log:

fetch("img/image_test.jpg").then(image => {
        console.log(image)
});


    ->Récupèrer depuis table posts fichier 1 de l' API JSON:

fetch("https://jsonplaceholder.typicode.com/posts/1")


    ->Utiliser méthode fetch pour récupèrer un fichier text sur un serveur distant, puis afficher le dans la console

fetch("https://jsonplaceholder.typicode.com/posts/")
.then(response => response.text())
.then(data => console.log(data))
.catch(error => console.log(error));


    ->Ecrire une fonction qui utilise la méthode fetch pour récupèrer des données JSON à partir d' une API, puis retpourne les données sous forme d' objets JS

function getJSONData(url){
    return fetch(url)
    .then(response => response.json())
    .then(data => console.log(data))
    .catch(error => console.log(error));
}
const jsonData = getJSONData("https://jsonplaceholder.typicode.com/posts/2")
console.log(jsonData);


->Utilisez la méthode fetch pour récupèrer une image à partir d'une URL, puis afficher la dans un élément img HTML:

const myImage = document.querySelector('img');
fetch('data:image/jpeg;base64,')
.then(function(response) {
  return response.blob();
})
.then(function(myBlob) {
  const objectURL = URL.createObjectURL(myBlob);
  myImage.src = objectURL;
});


->Récupèrer et afficher image du dossier dans balise image:

const myImage = document.querySelector('img');
fetch('img/image_test.jpg')
.then(function(response) {
  return response.blob();
})
.then(function(myBlob) {
  const objectURL = URL.createObjectURL(myBlob);
  myImage.src = objectURL;
});


->Écrivez une fonction qui utilise la méthode fetch pour envoyer des données à un
serveur distant en utilisant une méthode HTTP POST:

function envoiDoJs(url, user){
    const promise2 = fetch(url,
    {
    method: "POST",
    body: JSON.stringify(user),
    headers: {
        "Content-Type": "application/json"
    }
    }).then(response => console.log(response));
    console.log(promise2);
}
//test ok
const userTest = {
    name: "bahamut",
    mail: "bahamut-ju@gmail.com"
}
let urlTest = "https://jsonplaceholder.typicode.com/users";
envoiDoJs(urlTest, userTest);


->Utilisez la méthode fetch pour récupérer des données JSON à partir de l’API
themoviedb, puis utilisez ces données pour générer du contenu html en fonction
de ce que l’utilisateur aura entré comme titre dans un input que vous aurez prévu
à cet effet:








/*

a mettre dans main js general
Elle va utiliser mots clefs =>
-async= cette requete, fct, méthode est asynchrone.
-await= (dans fct)attend que la fonction, requete soit exécutée pour exécuter la suite
Exemple:
Faire fonction fetchData, doit retourner nouvelle promise
function fetchData(){
    return new promise(argument => {})
}
function fetchData(){
    return new promise(resolve =>{
        setTimeout(() => {
            resolve("Data has been fetched")
        }, 2000);
//imiter serveur lent
    })
}

function fetchData(){
    return new promise(resolve =>{
        setTimeout(() => {
            resolve("Data has been fetched")
        }, 2000);
    })
}
async function run(){
    let result = await fetchData();
    console.log(result);
}
async function run(){
    try{
        let result = await fetchData();
        console.log(result);
    }
    catch (error){
        console.error(error);
    }
}
run()
//ou
async function fetchData(){
    try{
        const response = await fetch("https:jsonplaceholder.typicode.com/posts/");
        const data = await response.json();
        console.log(data);
    }catch(error){
        console.error(error);
    }
}
fetchData();
//moins utilisée,permet chargement partiel de la page = petits paquets, que ça a fini ou non il commence a renvoyer les posts */


/* 
blob:
a mettre main js général
affichage image du dossier dans balise img HTML
Ici nous récupérons une image à travers le réseau et l'insérons dans un élément <img>.
L'utilisation la plus simple de fetch() prend un argument — le chemin de la ressource que nous souhaitons récupérer — et retourne une promesse (promise) contenant, en réponse, un objet (de type Response).
Bien sûr, il s'agit seulement d'une réponse HTTP, pas exactement de l'image. 
Pour extraire le contenu de l'image de la réponse, nous utilisons la méthode blob() (en-US) (définie sur le mixin Body, qui est implémenté autant par les objets Request que par les objets Response).
Un objet objectURL est ensuite créé à partir du Blob extrait, puis est inseré dans img.
Les requêtes Fetch sont controllées par la directive connect-src du Content Security Policy (en-US) plutôt que par la directive de la ressource dont il s’agit de la récupération.
*/

// fetch("https://jsonplaceholder.typicode.com/users")
//     .then(response => response.json())
//     .then(data => console.log(data))
//     .catch(error => console.log(error));

/*const newUser = {
    name: "anne-so",
    mail: "anneso-ju@gmail.com"
}
// objet js a envoyer -> à tranformer en json?
const promise1 = fetch("https://jsonplaceholder.typicode.com/users",
    {
    method: "POST",
    body: JSON.stringify(newUser),
// contenu de la requête post sera dans le body, il faut tranformer l' objet JS en JSON -> foncton JSON.stringify()
// objet JSON contient des méthodes pour interpréter du JSON (javascript object notation, et convertir des valeurs en JSON.)
// JSON est une syntaxe pour sérialiser des objets, des tableaux, nombres, chaînes de caractères... basée sur syntaxe JS mais distincte.
// JSON.stringify(valeur[, remplacement[, expace]])
// ->retourne une chaîne JSON correspondante à la valeur spécifiée
// =>ici conversion objet JS en JSON
// pour envoi au serveur?
    headers: {
        "Content-Type": "application/json"
    }
});
console.log(promise1);
// code statut 201 = created indique que la requête a réussi et qu'une ressource a été créee en conséquence.Généralement obtenu suite methode POST.
*/



